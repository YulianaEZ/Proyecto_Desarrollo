...
@bp.route('/activate', methods=('GET', 'POST'))#Se define la ruta de activación de la cuenta
def activate():#creación del método de activación de la cuenta
    try:
        if g.user:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'select * from activationlink where challenge = ? and status = ? ',(number,utils.U_UNCONFIRMED)
            ).fetchone()
